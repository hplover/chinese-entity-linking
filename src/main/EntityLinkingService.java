...
//    private HashMap<String, List<String>> result=new HashMap<>();
	...
	public EntityLinkingService(boolean debug){
		this.debug=debug;
	}
    ...
    private int disambiguation(Extract extObject, String keyword, String[] keywords){
    	String webpage=extObject.getContent().toString();
    	int target=0;
    	int start=webpage.indexOf("<div class=\"main-content\">");
    	String subpage=webpage.substring(start);
    	for(String kw:keywords){
    		if(!kw.equals(keyword)){
    			target=target+StringUtils.countMatches(subpage, kw);
    		}
    	}
    	return target;
    }
    ...
    public List<Result> entitylinking(String keywords_input){
    	String[] keywords=keywords_input.trim().split(" ");
        if(debug)
        ...
        for(String keyword:keywords){
            ...
            if(status==-1){
                ...
                String bkeLabel=bke.getLabel();
                if(debug)
                    System.out.println("label:"+bkeLabel);
                result.keywordURL=bkeURL;
                result.label=bkeLabel;
            }
                ...
                String tempPl="";
                String tempLabel="";
                Iterator it = bkePolysemant.entrySet().iterator();
                while (it.hasNext()) {
                    ...
                    int target=this.disambiguation(bke, keyword, keywords);
                ...
                result.keywordURL=tempUrl;
                result.keywordSynonym=tempPl;
	...
	public void entitylinking(String keywords_input,int mode){
    	this.results=wseg.segWord_full(keywords_input);
    	if(debug)
            System.out.println("-1(none) for entiry not found;\n0(Polysemants) for list page;\n1(Polysemant and content) for page with Polysemant;\n5(content and label and url not Polysemant maybe synonym) for unique page");
        for(Result rekeyword:this.results){
			...
			if(debug)
                System.out.println(keyword+"\n==============================================================");
//			if(!wseg.getP.contains(rekeyword.type)){
//				continue;
//			}
			bke.getResult(keyword, "");
			int status=bke.getStatus();
	...
	private void save_content(String keyword, String content){
        return ;
    }
    private void save_polysemant(String keyword, String polysemant){
    	return ;
    }
    private void save_synonym(String keyword, String synonym){
    	return ;
    }
...
//		String text="主任";
		EntityLinkingService entityLinkingService=new EntityLinkingService(true);
    	entityLinkingService.entitylinking(text,0);
